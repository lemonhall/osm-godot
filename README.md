...
**从真实世界地理数据生成 Godot Engine 3D 场景**

osm-godot 是一个 Rust 命令行工具，将 [OpenStreetMap](https://www.openstreetmap.org/) 的矢量数据和卫星高程数据转换为 Godot Engine 4.x 的 3D 场景文件。选择地球上任意一个区域，自动生成包含建筑、道路、树木、水域和真实地形的 Godot 项目。
...
ESA 地表分类 ──────→ 草地/森林/水域/建筑区
        │
        ▼
Godot 项目输出 ──→ .tscn 场景 + .tres 材质 + project.godot
```
...
├── default_environment.tres   # 默认环境（天空/光照）
...
- **引擎**：Godot 4.3+ (Forward Plus renderer)
- **数据源**：
