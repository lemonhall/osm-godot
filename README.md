...
- 路线指引：当前阶段保留连续绿色路线带，并在终点生成绿色到达圈；玩家进入圈内后会自动清掉路线带、到达圈和导航 HUD。先不生成路口 marker，也不触发语音播报。
- Godot 验证：
  ```powershell
...
- `G`：导航路线存在时开启/关闭自动巡航，速度使用玩家加速跑速度；手动移动会退出自动巡航
- `F`：查看眼前已加载建筑的本地 OSM 名称和关键属性，信息卡约 5 秒后自动隐藏
